...
"@
        ...
        Write-LogInfo "HTML transcript created at $transcriptPath"
    } catch {
        Write-LogError "Failed to create HTML transcript: $_"
    }
}
...
function Request-RebootIfNeeded {
    try {
        ...
        if ($pendingReboot) {
            Write-LogInfo "A system reboot is required to complete maintenance tasks."
            Write-LogInfo "Press any key to reboot. If no key is pressed in 120 seconds, this prompt will disappear and you will need to reboot manually."
            $timeout = 120
            $startTime = Get-Date
            $rebootNow = $false
            ...
            if ($rebootNow) {
                Write-LogInfo "Rebooting system..."
                Restart-Computer -Force
            } else {
                Write-LogInfo "No key was pressed. Please remember to reboot your system later."
            }
        } else {
            Write-LogInfo "No reboot is required."
        }
    } catch {
        Write-LogInfo "Failed to check or request reboot: $_"
    }
}
