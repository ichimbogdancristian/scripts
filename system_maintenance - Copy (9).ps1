...
<head>
    ...
    <style>
        ...
        .toc { background: #23272e; border-radius: 6px; padding: 1em; margin-bottom: 2em; position: sticky; top: 0; z-index: 10; }
        ...
        .legend { margin: 1em 0; padding: 0.5em 1em; background: #23272e; border-radius: 6px; }
        .collapsible { background: #23272e; color: #7fd1b9; cursor: pointer; padding: 0.5em 1em; width: 100%; border: none; text-align: left; outline: none; font-size: 1.1em; border-radius: 6px; margin-top: 1em; }
        .active, .collapsible:hover { background: #2d323a; }
        .content { padding: 0 1em; display: none; overflow: hidden; background: #23272e; border-radius: 0 0 6px 6px; }
        .task-stats { font-size: 0.95em; color: #b3b3b3; margin-bottom: 0.5em; }
...
<div class='container'>
    ...
    <div class='legend'>
        ...
        <span class='status-info'>ℹ️ Info</span>
    </div>
...
"@
        ...
        $summaryRows = @()
        $tocLinks = @()
        $totalTasks = 0; $totalSuccess = 0; $totalWarn = 0; $totalError = 0;

        if (-not ("System.Web" -as [type])) { Add-Type -AssemblyName System.Web }
        ...
        foreach ($log in $taskLogs) {
            ...
            # Only split camel case if the string is not all uppercase
            if ($readableTask -cmatch '[a-z]') {
                $readableTask = $readableTask -replace '([a-z])([A-Z])', '$1 $2'
            }
            # Capitalize first letter of each word, but keep rest as-is
            $readableTask = ($readableTask -split ' ' | ForEach-Object { if ($_.Length -gt 0) { $_.Substring(0,1).ToUpper() + $_.Substring(1) } else { $_ } }) -join ' '
            ...
            $statusIcon = '✅';
            $taskSuccess = 0; $taskWarn = 0; $taskError = 0; $taskInfo = 0;
            $htmlBody += "<div class='task' id='${anchor}'><h2>$readableTask</h2>"
            foreach ($m in $logMatches) {
                ...
                $statusClass = switch ($status) {
                    'SUCCESS' { $taskSuccess++; 'status-success' }
                    'ERROR'   { $taskError++; $statusSummary = 'ERROR'; $statusIcon = '❌'; 'status-error' }
                    'WARN'    { $taskWarn++; if ($statusSummary -eq 'SUCCESS') { $statusSummary = 'WARN'; $statusIcon = '⚠️' }; 'status-warning' }
                    'INFO'    { $taskInfo++; 'status-info' }
                    default   { '' }
                }
                $icon = switch ($status) {
                    'SUCCESS' { '✅' }
            ...
            $htmlBody += "<div class='task-stats'>Success: $taskSuccess | Warnings: $taskWarn | Errors: $taskError | Info: $taskInfo | Duration: $duration</div>"
            $htmlBody += "<button class='collapsible'>Show/Hide Raw Log</button><div class='content'><pre>$( [System.Web.HttpUtility]::HtmlEncode($logContent) )</pre></div>"
            $htmlBody += "</div>"
        ...
        $htmlBody = "<h2>Overall Summary</h2><div class='legend'>Total Tasks: $totalTasks | Success: $totalSuccess | Warnings: $totalWarn | Errors: $totalError</div>" + $htmlBody
        ...
        foreach ($file in $otherFiles) {
            ...
            $htmlBody += "<div class='file-section'><h3>File: $($file.Name)</h3><button class='collapsible'>Show/Hide Content</button><div class='content'><pre>$( [System.Web.HttpUtility]::HtmlEncode($fileContent) )</pre></div></div>"
        }
