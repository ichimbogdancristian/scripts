...
function Write-Log {
    ...
    # Use ANSI RGB codes for all log levels
    $reset = "`e[0m"
    $color = switch ($Level) {
        'INFO'    { "`e[38;2;102;204;255m" }      # Light blue (custom for INFO)
        'SUCCESS' { "`e[38;2;0;200;83m" }         # Green (custom for SUCCESS)
        'WARNING' { "`e[38;2;246;133;55m" }       # Orange (#F68537)
        'ERROR'   { "`e[38;2;255;71;87m" }        # Red (custom for ERROR)
...
function Write-SectionHeader {
    ...
    $rgb = "\e[38;2;246;133;55m"  # #F68537
    $reset = "\e[0m"
    Write-Host "`n$($rgb)$line$($reset)"
    Write-Host ("     [ $Title ]     ") -NoNewline
    Write-Host "" -ForegroundColor White
    Write-Host "$($rgb)$line$($reset)"
}
...
function Invoke-Task {
    ...
    try {
        & $TaskScript
        # Write-Log "$TaskName completed successfully" 'SUCCESS'  # Removed duplicate success log
    } catch {
...
function Invoke-Task3_PackageManagerSetup {
    ...
    try {
        ...
        # Write-Log "Task 3: Package Manager Setup completed successfully." 'SUCCESS'  # Removed duplicate success log
    } catch {
...
function Invoke-Task5_RemoveBloatware {
    ...
    try {
        ...
        if (Test-Path $installedProgramsDiffPath) {
            ...
            Write-Log "Deleted temp file: $installedProgramsDiffPath" 'INFO'
        }
    } catch {
...
function Invoke-Task7_UpgradeAllPackages {
    ...
    Write-Host "[Upgrade] Checking for upgradable packages via winget and choco..."
    try {
        $wingetUpgraded = @()
        $chocoUpgraded = @()
        $wingetFailed = @()
        ...
        if (Get-Command winget -ErrorAction SilentlyContinue) {
            $wingetListJson = winget upgrade --source winget --output json 2>&1 | Out-String
            $wingetList = @()
            try {
                $wingetList = $wingetListJson | ConvertFrom-Json
            } catch {
                Write-Host "[Upgrade] Failed to parse winget upgrade JSON output."
                $wingetList = @()
            }
            $wingetPackages = @()
            foreach ($pkgObj in $wingetList) {
                if ($pkgObj.Id) { $wingetPackages += $pkgObj.Id }
            }
            Write-Host ("[Upgrade] The following packages will be upgraded via winget: {0}" -f ($wingetPackages -join ', '))
            $wingetTotal = $wingetPackages.Count
            $wingetCurrent = 0
            foreach ($pkg in $wingetPackages) {
                $wingetCurrent++
                Write-Progress -Activity "Winget Upgrade" -Status ("Upgrading: {0} ({1}/{2})" -f $pkg, $wingetCurrent, $wingetTotal) -PercentComplete ([int](($wingetCurrent / $wingetTotal) * 100))
                Write-Host ("[Upgrade] Upgrading {0} via winget..." -f $pkg)
                try {
                    $transcript += "[{0}] Upgrading $pkg via winget..." -f ((Get-Date).ToString('HH:mm:ss'))
                    $result = winget upgrade --id $pkg --accept-source-agreements --accept-package-agreements --silent -e 2>&1
                    $transcript += $result
                    if ($result -match 'No applicable update found' -or $result -match 'No installed package found') {
                        Write-Host ("[Upgrade] No update found for {0} via winget." -f $pkg)
                        $wingetFailed += $pkg
                    } else {
                        Write-Host ("[Upgrade] Successfully upgraded {0} via winget." -f $pkg)
                        $wingetUpgraded += $pkg
                    }
                } catch {
                    Write-Host ("[Upgrade] winget failed for {0}: {1}" -f $pkg, $_)
                    $transcript += "[{0}] [ERROR] winget failed for {1}: {2}" -f ((Get-Date).ToString('HH:mm:ss')), $pkg, $_
                    $wingetFailed += $pkg
                }
            }
            ...
            Write-Host "[Upgrade] winget not found. Skipping package upgrade."
            $transcript += "[{0}] [WARN] winget not found. Skipping package upgrade." -f ((Get-Date).ToString('HH:mm:ss'))
        }
        # Now use choco only for packages that failed with winget
        if (Get-Command choco -ErrorAction SilentlyContinue) {
            $chocoList = choco outdated -r 2>&1 | Where-Object { $_ -and $_ -notmatch '^Title|^$' }
            $chocoPackages = @()
            foreach ($line in $chocoList) {
                $pkg = $line.Split('|')[0]
                if ($pkg -and $pkg -ne 'Package Name') { $chocoPackages += $pkg }
            }
            $toChoco = $chocoPackages | Where-Object { $wingetUpgraded -notcontains $_ }
            Write-Host ("[Upgrade] The following packages will be upgraded via choco: {0}" -f ($toChoco -join ', '))
            $chocoTotal = $toChoco.Count
            $chocoCurrent = 0
            foreach ($pkg in $toChoco) {
                $chocoCurrent++
                Write-Progress -Activity "Choco Upgrade" -Status ("Upgrading: {0} ({1}/{2})" -f $pkg, $chocoCurrent, $chocoTotal) -PercentComplete ([int](($chocoCurrent / $chocoTotal) * 100))
                Write-Host ("[Upgrade] Upgrading {0} via choco..." -f $pkg)
                try {
                    $transcript += "[{0}] Upgrading $pkg via choco..." -f ((Get-Date).ToString('HH:mm:ss'))
                    $result = choco upgrade $pkg -y 2>&1
                    $transcript += $result
                    if ($result -match '0 upgraded' -or $result -match 'not upgraded') {
                        Write-Host ("[Upgrade] No update found for {0} via choco." -f $pkg)
                        $transcript += "[{0}] [WARN] $pkg not upgraded by choco." -f ((Get-Date).ToString('HH:mm:ss'))
                    } else {
                        Write-Host ("[Upgrade] Successfully upgraded {0} via choco." -f $pkg)
                        $chocoUpgraded += $pkg
                    }
                } catch {
                    Write-Host ("[Upgrade] choco failed for {0}: {1}" -f $pkg, $_)
                    $transcript += "[{0}] [ERROR] choco failed for ${pkg}: $_" -f ((Get-Date).ToString('HH:mm:ss'))
                }
            }
            Write-Progress -Activity "Choco Upgrade" -Status "All packages processed" -Completed
            $transcript += "[{0}] All possible remaining packages processed via choco." -f ((Get-Date).ToString('HH:mm:ss'))
        } else {
            Write-Host "[Upgrade] choco not found. Skipping Chocolatey package upgrade."
            $transcript += "[{0}] [WARN] choco not found. Skipping Chocolatey package upgrade." -f ((Get-Date).ToString('HH:mm:ss'))
        }
        Write-Host ("[Upgrade] Packages upgraded via winget: {0}" -f ($wingetUpgraded -join ', '))
        Write-Host ("[Upgrade] Packages upgraded via choco: {0}" -f ($chocoUpgraded -join ', '))
        Write-Host ("[Upgrade] Packages not upgraded: {0}" -f ($wingetFailed -join ', '))
        $transcript += "[{0}] [SUCCESS] Upgrade All Packages" -f ((Get-Date).ToString('HH:mm:ss'))
    } catch {
        Write-Host ("[Upgrade] Task 7 failed: {0}" -f $_)
...
function Invoke-Task9_WindowsUpdate {
    ...
    try {
        ...
        # Write-Log "Task 9: Windows Update & Upgrade completed successfully." 'SUCCESS'  # Removed duplicate success log
    } catch {
...
"@
        ...
        $taskFiles = @(
            ...
            @{ Name = 'Remove Bloatware'; File = 'Task5_RemoveBloatware_log.txt' },
            @{ Name = 'Install Essential Applications'; File = 'Task6_InstallEssentialApps_log.txt' },
            @{ Name = 'Upgrade All Packages'; File = 'Task7_UpgradeAllPackages_log.txt' },
            @{ Name = 'Privacy & Telemetry'; File = 'Task8_PrivacyAndTelemetry_log.txt' },
            @{ Name = 'Windows Update'; File = 'Task9_UpdatesMaintenance_log.txt' },
            @{ Name = 'Cleanup'; File = 'Task10_Cleanup_log.txt' },
...
"@
        # Save transcript in the local script folder
        $scriptFolder = $PSScriptRoot
        $outPath = Join-Path $scriptFolder 'SystemMaintenance_Transcript.html'
        $htmlContent = $htmlHeader + $htmlBody + $htmlFooter
        $htmlContent | Out-File $outPath -Encoding UTF8
        # Write-Log "Transcript saved to $outPath" 'SUCCESS'  # Removed duplicate success log
    } catch {
        Write-Log "Failed to export transcript: $_" 'ERROR'
    }
}
