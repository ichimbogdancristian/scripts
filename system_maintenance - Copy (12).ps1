    ...
    try {
        ...
        foreach ($bloat in $bloatwareToRemove) {
            ...
            foreach ($match in $bloatMatches) {
                ...
                $triedWingetAfterAppxFail = $false
                if ($pkg) {
                    $methodsTried += 'AppX'
                    try {
                        Remove-AppxPackage -Package $pkg.PackageFullName -AllUsers -ErrorAction Stop
                        ...
                        # Failover: Try winget if AppX removal fails
                        ...
                        if ($wingetId -and (Get-Command winget -ErrorAction SilentlyContinue)) {
                            $methodsTried += 'winget-after-appx-fail'
                            try {
                                $wingetResult = winget uninstall --id "$wingetId" --exact --silent --accept-source-agreements --accept-package-agreements 2>&1
                                if ($wingetResult -notmatch 'No installed package found') {
                                    Write-Log "Uninstalled via winget (failover): $wingetId" 'SUCCESS'
                                    $uninstallSuccess = $true
                                } else {
                                    Write-Log "Winget could not uninstall (failover): $wingetId" 'WARNING'
                                }
                            } catch {
                                Write-Log ("winget uninstall (failover) failed for {0}: {1}" -f $wingetId, $_) 'WARNING'
                            }
                            $triedWingetAfterAppxFail = $true
                        }
                    }
                }
                # 2. Try winget uninstall (use robust mapping if available), but only if not already tried as failover and uninstallSuccess is still $false
                if (-not $triedWingetAfterAppxFail -and -not $uninstallSuccess) {
                    $wingetId = if ($BloatwareSpecialCases.ContainsKey($bloat) -and $BloatwareSpecialCases[$bloat].Winget) { $BloatwareSpecialCases[$bloat].Winget } else { $match }
                    if ($wingetId -and (Get-Command winget -ErrorAction SilentlyContinue)) {
                        $methodsTried += 'winget'
                        try {
...
function Invoke-Task7_UpgradeAllPackages {
    ...
    try {
        ...
        if (Get-Command winget -ErrorAction SilentlyContinue) {
            $wingetList = winget upgrade --accept-source-agreements --accept-package-agreements --source winget 2>&1 | Select-Object -Skip 1
            $wingetPackages = @()
            foreach ($line in $wingetList) {
                if ($line -match '^(\S+)\s+') {
                    $pkg = $line.Split()[0]
                    if ($pkg -and $pkg -ne 'Name') { $wingetPackages += $pkg }
                }
            }
...
"@
        ...
        Write-Log "Transcript saved to $outPath" 'SUCCESS'
    } catch {
